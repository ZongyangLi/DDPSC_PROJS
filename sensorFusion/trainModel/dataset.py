...
class OPENFusionJointDataset(Dataset, LabelsQueriable):
    def __init__(self, open_dataset_root, season=4, start_date=None, end_date=None,exclude_cultivar=None, exclude_date=None, transform=None):
        ...
        self.rgb_dataset_root = os.path.join(open_dataset_root, 'rgb')
        self.thermal_dataset_root = os.path.join(open_dataset_root, 'thermal')
    ...
    def _make_dataset(self):
        ...
        paths = []
        labels = {'plot': [], 'scan_date': [], 'sensor':[]}
        file_path_list = glob.glob(os.path.join(self.rgb_dataset_root, '**', '*{}'.format(self.image_file_suffix)))
        for file_path in file_path_list:
            ...
            labels['sensor'].append('S')
            
        file_path_list = glob.glob(os.path.join(self.thermal_dataset_root, '**', '*{}'.format(self.image_file_suffix)))
        for file_path in file_path_list:
            path_split = file_path.split('/')
            plot = path_split[-2]
            scan_date = datetime.strptime(path_split[-1][:10], '%Y-%m-%d').date().toordinal()
            ...
            labels['plot'].append(plot)
            labels['scan_date'].append(scan_date)
            labels['sensor'].append('F')
            
        le = LabelEncoder()
        plot_int = le.fit_transform(labels['plot'])
        labels['plot_int'] = plot_int
...
class OPENFusionJointDataset_test(Dataset, LabelsQueriable):
    def __init__(self, open_dataset_root, season=4, start_date=None, end_date=None,exclude_cultivar=None, exclude_date=None, transform=None):
        self.open_dataset_root = open_dataset_root
        self.rgb_dataset_root = os.path.join(open_dataset_root, 'rgb')
        self.thermal_dataset_root = os.path.join(open_dataset_root, 'thermal')
